...
VS_OUTPUT VS(VS_INPUT input){
	...
	for (int i = 0; i < 4; i++)
	{
		...
		{
			float4 pos = input.BlendWeights[i] * mul(originalPosition, gBones[input.BlendIndices[i]]);
			transformedPosition += pos;
			
			float3 norm = input.BlendWeights[i] * mul(input.normal, (float3x3)gBones[input.BlendIndices[i]]);
