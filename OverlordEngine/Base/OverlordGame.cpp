...
bool OverlordGame::m_ShouldQuit = false;
...
HRESULT OverlordGame::Run(HINSTANCE hInstance)
{
	...
	while(msg.message != WM_QUIT && !m_ShouldQuit)
	{
...
{
	//Clear Backbuffer
	m_pDeviceContext->ClearRenderTargetView(m_pCurrentRenderTarget->GetRenderTargetView(), reinterpret_cast<const float*>(&Colors::Gray));
